...
public class GeometryFactory {
    ...
    /**
     * Erzeugt eine Kugel mit verschobenen Vertices.
     * @param r Radius der Kugel
     * @param n Anzahl der vertikalen Streifen
     * @param k Anzahl der horizontalen Streifen
     * @param displaceHeight Maximale Verschiebung in Modelkoordinaten
     * @param displaceFile Pfad zu einer Bilddatei mit Höheninformationen
     * @return Geometrie der Kugel
     */
    public static Geometry createSphereWithDisplacement(float r, int n, int k, float displaceHeight, String displaceFile) {
        ...
        for(int j=0; j <= k; ++j) {
            ...
            float phi = 0;
            for(int i=0; i <= n; ++i) {
                float height = displaceHeight * image[(int)((theta / Util.PI) * (float)image.length) % image.length]
                ...
                // position
                fb.put((r + height) * sinTheta * cosPhi);  
                fb.put((r + height) * cosTheta);
                fb.put((r + height) * sinTheta * sinPhi);
