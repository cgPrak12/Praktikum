...
//	private static float param = 0.0f;
	...
	public static void main(String argv[])
	{
		try
		{
			...
			terra = new util.Terrain(2.75f, 4096, 4096, 4);

			terra.genTerrain(8);
			float[][][] dummy = terra.getTerra();
			float min = 1000;
			float max = -1000;
			for(int i=0; i< dummy.length; i++){
				for(int j=0; j< dummy.length; j++){
					if(min>dummy[i][j][0]) min = dummy[i][j][0];
					if(max<dummy[i][j][0]) max = dummy[i][j][0];
				}
					
			}
			System.out.println(max);
			System.out.println(min);
			clip = new ClipMap(1022, 8, program, cam);
...
//			program.setFloat("param", param);
	...
	{
		float moveSpeed = 2e-3f * (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) ? 500.0f : 100.0f) * (float) millis;
