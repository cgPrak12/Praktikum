...
public class MainTest
{
	...
	private static ShaderProgram shaderProgramTerrain;
	private static ShaderProgram shaderProgramModels;
    ...
    public static void main(String argv[]) {
        try {
            ...
            clip = new ClipMap(254, 14, shaderProgramTerrain, cam);
            cam.setClipMap(clip);
	...
	long last = System.currentTimeMillis();
	    ...
	    shaderProgramTerrain.setUniform("viewProj", Util.mul(null, cam.getProjection(), cam.getView()));
	    shaderProgramTerrain.setUniform("model", terrainModelMatrix);
            shaderProgramTerrain.setUniform("elevation", tex);
            shaderProgramTerrain.setUniform("materials", materialTex);
                ...
                for(int z=0; z<modelMap.length; z++) {
                    if(modelMap[x][z]!=null && modelMap[x][z].getModelList()!=null) {
                        ...
                        while(modelListIterator.hasNext()) {
                            ...
                            shaderProgramModels.setUniform("model", new Matrix4f());
                            shaderProgramModels.setUniform("viewProj", Util.mul(null, cam.getProjection(), cam.getView()));   
