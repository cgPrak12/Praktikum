...
//	float sample= texture2D(scene,texCoords + j*vec2(1.0/size, 0)).w;
	...
	sample = sample + texture2D( scene, texCoords + j*vec2( 0.0, 1.0/size) ) * vec4(factor)
		           + texture2D( scene, texCoords - j*vec2( 0.0 , 1.0/size) ) * vec4(factor);
	factor= factor/2.0;	           
	...
	sum+=(sample*vec4(g_x,g_y,g_z,g_w)*vec4(0.7));
	i+=(w*g_w)*2.0;
}
