...
//    private Geometry geo = new Geometry();
    ...
    private int GLheightandnormalmapID;

    private CLMem heightnormalmap;
    ...
    public void createData(int heightandnormalmap){
    	
        this.GLheightandnormalmapID = heightandnormalmap;
    	...
    	for(int i=0; i < MAX_PARTICLES; i++){
    		particles.put((float)Math.random());//(float)(Math.random()));
    		particles.put((float)Math.random());//0.1f + (float)i*0.001f);
    		particles.put((float)Math.random());//(float)(Math.random())*0.5f);
    		particles.put(1f);
    	}
...
//        
        ...
        clEnqueueAcquireGLObjects(this.queue, this.heightnormalmap, null, null);
        ...
        clEnqueueReleaseGLObjects(this.queue, this.heightnormalmap, null, null);
...
//        System.out.println("Summe = " + sum);
    ...
    private void createKernels() {
        ...
        heightnormalmap = CL10GL.clCreateFromGLTexture2D(this.context, 
                                                   CL10.CL_MEM_READ_ONLY,
                                                   GL11.GL_TEXTURE_2D,
                                                   0,
                                                   GLheightandnormalmapID, errorCheck);
        
        OpenCL.checkError(errorCheck.get(0));
        
        this.kernel0.setArg(2,heightnormalmap);
        this.kernel0.setArg(3,this.gridCounters);
        this.kernel0.setArg(4,this.gridCells);
        this.kernel0.setArg(5,this.gridLen);
        this.kernel0.setArg(6,this.gridMaxParticles);
        this.kernel0.setArg(7, 0.0f); // dt, see draw()
        this.kernel0.setArg(8, 0.0f);
        this.kernel0.setArg(9, this.new_pos);
        this.kernel0.setArg(10, this.new_velos);
        this.kernel0.setArg(11,this.gridInfo);
        this.kernel0.setArg(12, this.start_pos);
