...
public class ClipMap
{
	...
	private Matrix4f translation; // Translationsmatrix f�r Shaderprogramm
	...
	private float tempX; // Variable f�r Bewegungsschwellenwert
	private float tempY; // Variable f�r Bewegungsschwellenwert
	private float tempZ; // Variable f�r Bewegungsschwellenwert
	 ...
	 * 
	 * @param size Gr��e der ClipMap (Anzahl der K�stchen)
	 * @param stage Anzahl der Aufl�sungslevel
	 * @param program Dazugeh�riges Shaderprogram
	...
	public void generateMaps()
	{
		...
		// Z�hle Floats hoch, bis Schwellenwert erreicht ist
...
//			TerrainView.updateTerrainView();
	...
	private void updateHeight(boolean mode)
	{
		if (mode)
		{
			if (scaleSteps < 3)
			{
				...
				System.out.println("Gr��e Movement" + movement[0][0] + " bei Aufl�sungslevel " + scaleSteps);
			}
		} else
		{
			if (scaleSteps > 0)
	...
	public void moveClip(int i, int dir)
	{
		if (dir == 0 || dir == 1) // Unterscheidung der Bewegungsrichtung: pos X
									// & pos Y
		{
			if (i == stage - 1) // Abbruchbedingung wenn der �u�erste
			...
			} else
			{
				...
				// Bewegungsrichtung bis Platz ist oder �u�erster Ring erreicht
				{
					alignment[i][dir] ^= true;
					alignment[i][dir + 2] ^= true;
					movement[i][dir] += 2;
		...
		{
			if (dir == 2 || dir == 3) // Analog f�r neg X & neg Y
			{
				if (i == stage - 1)
				{
					movement[i][dir - 2] -= 2;
	...
	/** Erzeugt die f�r eine ClipMap ben�tigten L-Geometrien und zeichnet diese
	 * lageabh�ngig
	 * 
	...
	private void setLGrid(int i)
	{
		...
		int side = 0; // Lage der L Geometrien, abh�ngig von der Lage bestimmen
	...
	{
		program.setFloat("heightScale", (float)((stage * (size + 2) / 16) + 12f)*generalScale);
	}
