...
//        private final static int detailsteps = 20;
     ...
     // Festgesetzte Variablen fuer Ausgabe
    	 ...
    	 * @return
    	 */
    	public static float[][][][] minGrid(float[][][] src, Camera cam)
    	{
    	 ...
    	 * @return
    	 */
    	public static float[][][][] minGrid(float[][][] src, Camera cam, int size)
    	{
    		// false-values
    		if(src == null){
...
//    			return new FloatBuffer[0];
          ...
          // die Kamera Position ist naemlich nicht an den Stellen wie in der src
    		...
    		// set result[][][][]
    		density = updateHeight(src);
    		result = new float[size+1][2*density+1][2*density+1][7];
    		
    		infos = new int[2+2*size];
    		infos[0] = density;
    		infos[1] = size;
...
//    		return getFloatBufferGrid(result);
    	...
    	private static float[][][] getBlock(float[][][] src, int size, int density){
    		...
    		float[][][] area = new float[2*density+1][2*density+1][7];
...
//    					area[i][j] = dummy;
    					...
    					area[i][j][0] = x;
    					area[i][j][1] = src[x][z][0];
    					area[i][j][2] = z;
    					area[i][j][3] = src[x][z][1];
    					area[i][j][4] = src[x][z][2];
    					area[i][j][5] = src[x][z][3];
    					area[i][j][6] = src[x][z][4];
//    					
//    					area[i][j] = src[x][z];
    				}
    				j++;
        ...
        /**
         * 
         * @param xPos camera-positoin
         * @param zPos
         * @return
         */
        public static float[][][][] update(int xPos, int yPos, int zPos){
         
        	camPosHeight = yPos;
        	int diffX = camPosX - xPos;
        	int diffZ = camPosZ - yPos;
        	
//        	if(Math.abs(density - updateHeight()) 
